...
class Register implements \ArrayAccess{
	...
	public static function set($name,$class){
		self::_registerService($name, $class);
	}

	//设置共享服务 会让该服务成为单例形式
	public static function setShared($name,$class){
		self::_registerService($name, $class, true);
	}

	//注册服务
	private static function _registerService($name,$class,$shared=false){
		//移除之前同名的函数
		self::remove($name);
		...
		if(!($class instanceof \Closure) && is_object($class)){
			...
			//添加到服务列表
			self::$_bindings[$name] = array("class"=>$class,"shared"=>$shared);
		}
	}
