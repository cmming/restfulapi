...
class CoreLogger
{
	...
	const LOG_LEVL_DBDATA = 0x10;			//数据库操作信息
	...
	private function __construct()
    {
		...
		$this->_logPath = ConfigAutoReload::getInstance('\\Configs\\')['ConstConfig']['log_path']['path_sys_log'];
		//设置数据文件
		//$this->_dataPath = PATH_SYS_DATA;
		$this->_dataPath = ConfigAutoReload::getInstance('\\Configs\\')['ConstConfig']['log_path']['path_sys_data'];
    ...
    public function writeLog($location, $message,$level= self::LOG_LEVL_DEBUG,$utime = true,$need_echo = false)
    {
		...
		}else if($level==self::LOG_LEVL_DBDATA)
		{
			//数据文件单独存
			//目录不存在，则创建
			if(!file_exists($this->_logPath))
			...
			$logFile = $this->_dataPath.$this->_dataFileName.".sql";
			$msg = "[".date('Y-m-d H:i:s')."][".$location."][pid=".getmypid()."]".$message;
			self::writeFileLock($logFile,$msg);
		}
